...
def validate_ground(Question, rule_id: int, question, answer, *, verbose=False):
...
def ground_and_revise_in_one_batch(Question, rules, question, answer):
    messages=[
        {
            'role': 'user',
            'content': prompt.format(
...
def ground_and_revise(Question, question, answer, *, max_batch) -> Tuple[List[int], str]:
    IDs = []
    for i in range(0, min(max_batch, len(batched_rules))):
        response = ground_and_revise_in_one_batch(Question, batched_rules[i], question, answer)
        print(f'Batch {i+1}')
        print(response)
        ...
        if ref == 'EMPTY' or ref == '':
            continue
        ref = int(ref)
        if validate_ground(Question, ref, question, answer, verbose=True):
            IDs.append(ref)
